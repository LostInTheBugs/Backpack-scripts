...
def get_combined_signal(df, symbol):
    """
    Version sync qui gère le cas où compute_all retourne une coroutine
    """
    ...
    try:
        ...
        # Si c'est une coroutine, on ne peut pas la traiter dans une fonction sync
        if asyncio.iscoroutine(result):
            from utils.logger import log
            log(f"[ERROR] [{symbol}] compute_all returned coroutine but get_combined_signal is sync", level="ERROR")
            log(f"[ERROR] [{symbol}] Need to make get_combined_signal async or use sync version of compute_all", level="ERROR")
            return "HOLD"
