...
async def async_main(args):
    ...
    if args.auto_select:
        ...
        if args.no_limit:
            ret = subprocess.run(["python3", "V2/ScriptDatabase/fetch_top_n_volatility_volume.py", "--no-limit"])
        else:
            # Tu peux ajuster ici le nombre par défaut, ex 10
            ret = subprocess.run(["python3", "V2/ScriptDatabase/fetch_top_n_volatility_volume.py", "10"])
