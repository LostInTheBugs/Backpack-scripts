...
async def scan_all_symbols(pool, symbols):
    log(f"[DEBUG]🔎 Scan initial de {len(symbols)} symboles...", level="DEBUG")
    for symbol in symbols:
        ...
        try:
            ...
            if not exists:
                log(f"[DEBUG]  - Données manquantes pour {symbol}, déclenche récupération", level="DEBUG")
                # ici appelle ta fonction de récupération si besoin
        except Exception as e:
            log(f"[ERROR] Erreur lors du scan initial {symbol}: {e}", level="ERROR")
    log("[DEBUG] ✅ Scan initial terminé.", level="DEBUG")
