...
async def scan_all_symbols(pool, symbols):
    log(f"🔎 Scan initial de {len(symbols)} symboles...")
    for symbol in symbols:
        ...
        try:
            ...
            if not exists:
                log(f"  - Données manquantes pour {symbol}, déclenche récupération")
                # ici appelle ta fonction de récupération si besoin
        except Exception as e:
            log(f"Erreur lors du scan initial {symbol}: {e}")
    log("✅ Scan initial terminé.")
